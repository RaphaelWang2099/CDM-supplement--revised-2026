# =====================================================================
# small pipe 5  (2026-08-01)  —  基於 small pipe 4 的 Ward 修正版
# 修改内容（僅涉及層次聚類，其餘與 small pipe 4 完全一致）：
#   1. Ward 聚類距離：1-cos → sqrt(2(1-cos))（弦距離 = L2 歸一化向量的
#      歐氏距離）。SciPy 的 method="ward" 僅在歐氏度量下定義正確；
#      修正後等價於對 L2 歸一化 3-gram TF 向量做標準 Ward。
#   2. 圖 5 說明框與標題同步改寫；k 截斷線 2/4/6 → 2/5/6。
#   3. 聚類報告（docx）中 Ward 描述與 k 分群列表（[2,4]→[2,5]）同步。
#   注：MDS 圖仍用 1-cos 作非度量相異度（MDS 不要求歐氏），未改動。
# ---------------------------------------------------------------------
# small pipe 5.1 (2026-08-01)：在 5 的基礎上，僅將圖內說明框的僞代碼
# 公式改爲 matplotlib mathtext 標準數學排版（Ward／MDS／PCA／Strength
# 四處），並使圖4公式與刊234注⑥統一爲 cos_raw、j≠i。算法零改動。
# ---------------------------------------------------------------------
# small pipe 5.3 (2026-08-03)：校勘記同字重出合併，算法與數值零改動。
#   _diff_summary_han 的「校勘记」notes：同一 (tag, A段, B段) 合併爲
#   一條，出現次數 ≥2 時追加「（凡N見）」；順序按首次出現位置；
#   replace 塊按字拆分計次（塊內逐字 1:1），delete/insert 成句塊不拆；
#   max_items 截斷改按合併後條數計。dist/rep/ins/dele 與 norm_edit
#   等一切計數不變（仍按字符位逐位計）。其餘與 5.2 完全一致。
#   （2026-08-03 續修）合併改爲可選開關 merge_repeated_variants，默認關閉：
#   默認輸出逐字位原始條目（忠實機器輸出原則），開啓時方合併並記「（凡N見）」。
# ---------------------------------------------------------------------
# small pipe 5.2 (2026-08-02)：顯示標籤繁體歸一，算法與數值零改動。
#   1. 讀表時對 書名/篇名（h1/h2）標籤作簡→繁歸一（_normalize_label_tc）；
#      正文（text）爲實驗語料，一字未動，標籤不參與任何相似度計算。
#   2. _ANALECTS_CHAPTERS 與 _ANALECTS_CHAPTER_ALIAS 規範形改爲繁體
#      （鄉黨/顏淵/憲問/堯曰），修復樹狀圖葉標籤等處的簡體字形。
#   驗證：與 5.1 舊輸出逐行 diff，除標籤繁體與時間戳外零差異；
#   五值 0.1991/0.0496/0.0521/0.0531/0.0452、PCA、簇成員均不變。
# ---------------------------------------------------------------------
# 5.3（續）圖版文字修訂（自 5.2b 併入），算法與數值零改動。
#   1. PCA 説明框放寬（note_box_width_scale 0.70→1.0）並折行，修文字溢框；
#   2. PCA 解讀句以實測解釋方差（8.0%）替換「合計>50% 爲良好投影」模板句；
#   3. Ward 樹狀圖標題去掉「（歐氏距離，L2 歸一化）」括注（距離定義仍在框內）；
#   4. 圖框/報告文字 為→爲（大陸正字）。
#   5. (08-03) Ward 樹狀圖去 k=6 截斷線與圖例（k=6 未用於任何結論），僅存 k=2/k=5。
#   6. (08-04) PCA 説明框去「投影僅作輔助觀察」句（解釋方差已標於座標軸）。
# =====================================================================# =====================================================================
import os
import re
import math
...
PIPELINE_CODE_VERSION = "small_pipe54_pca_label_relax_v1"
...
# 2026-08-02 rev2：書名/篇名顯示標籤繁體歸一。
# 源表個別標籤爲簡體（论/尧/颜/渊/问/党），致樹狀圖葉標籤、簇表、報告出現簡體字形。
# 僅作用於 h1/h2 標籤欄；正文（text）爲實驗語料，嚴禁改動——標籤不參與任何相似度計算，
# 故本歸一不影響任何數值輸出。
_LABEL_TC_MAP = str.maketrans({
    "论": "論", "语": "語", "尧": "堯", "颜": "顏", "渊": "淵",
    "问": "問", "党": "黨", "宪": "憲", "乡": "鄉", "记": "記",
    "传": "傳", "选": "選", "为": "爲", "书": "書",
})
...
def _normalize_label_tc(s: str) -> str:
    return s.translate(_LABEL_TC_MAP)
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        units.append(
            Unit(
                ...
                h1="" if h1 is None else _normalize_label_tc(str(h1).strip()),
                h2="" if h2 is None else _normalize_label_tc(str(h2).strip()),
                text=txt,
                para_ids=[uid],
...
_ANALECTS_CHAPTERS = [
    "學而", "爲政", "八佾", "里仁", "公冶長", "雍也", "述而", "泰伯", "子罕", "鄉黨",
...
_CLUSTER_FIXED_STYLE: Dict[str, Dict[str, float]] = {
    ...
    "pca": {
        ...
        "note_box_width": 0.92,
        "note_min_ratio": 0.22,
        # 5.2b：0.70 的窄框放不下首行公式，文字溢出框外；放寬至全幅並配合
        # info_text 折行（見 plot_pca_biplot）。
        "note_box_width_scale": 1.0,
    },
    "strength": {
        "width_ratio": 13.0,
        "height_ratio": 16.0,
...
def plot_cluster_dendrogram(sim_matrix, doc_names, outpath, font_setting=""):
    ...
    # small pipe 5: Ward 要求歐氏度量；對 L2 歸一化向量，歐氏距離 = sqrt(2(1-cos))（弦距離）
    dist = np.sqrt(2.0 * dist)
    ...
    info_text = (
        ...
        "━━━━━━━━━━━━━━━━━━\n"
        "合併距離公式：$d(A,B)=\\sqrt{2|A||B|/(|A|+|B|)}\\;\\Vert c_A-c_B\\Vert$\n"
        "算法原理：自底向上逐步合併距離最近的簇，每次合併選擇使組內方差增量最小的一對。\n"
        "距離矩陣：$d(i,j)=\\sqrt{2\\,(1-\\cos_{\\mathrm{raw}}(i,j))}$，即 L2 歸一化 3-gram TF 向量的歐氏距離\n"
    ...
    # k=2, k=5 截断线（5.3 續：k=6 未用於任何結論，去除以免誤導）
...
def plot_mds_scatter(sim_matrix, doc_names, outpath, font_setting=""):
    ...
    info_text = (
        ...
        "━━━━━━━━━━━━━━━━━━\n"
        "目標函數（Stress）：$s=\\sqrt{\\sum_{i<j}(d_{ij}-\\hat{d}_{ij})^2\\,/\\,\\sum_{i<j}d_{ij}^2}$\n"
        "算法原理：將高維距離矩陣投影到二維平面，盡量保留文本間的相對距離關係。\n"
        "距離定義：$d(i,j)=1-\\cos_{\\mathrm{raw}}(i,j)$\n"
        "解讀：距離近=用詞相似度高；聚集區域=同類文本群；stress < 0.1 爲良好擬合"
    )
...
def _pca_relax_labels(fig, ax, coords, anns, marker_size, vis_scale):
    """PCA 圖標籤去壓印（5.4 新增）。

    確定性貪心避讓：按索引順序逐個放置，先試原位（故不擁擠處與 5.3 完全一致），
    衝突時依固定候選序改用其他方位；候選框由首次量得的文字尺寸平移而得，
    不重繪、不引入隨機性，圖可精確復現。
    """
    if not anns:
        return
    ...
    pad = 1.6 * vis_scale                      # 標籤間留白（像素）
    m_half = (float(marker_size) ** 0.5) * vis_scale * 0.62   # 標記半徑（像素）
    pts = ax.transData.transform(coords)       # 各點的顯示座標

    # 候選方位（dx, dy, ha）：原位 → 下方 → 左側 → 更遠的上/下 → 純上/下
    def _cands(dx0, dy0, ha0):
        alt = "right" if ha0 == "left" else "left"
        s = 1.0 if ha0 == "left" else -1.0
        base = abs(dx0)
        return [
            (dx0, dy0, ha0),
            (dx0, -dy0 - 7.0, ha0),
    ...
    def _box(bb0, ax_, ay_, dx0, dy0, ha0, dx, dy, ha):
        w, h = bb0.width, bb0.height
        voff = bb0.y0 - (ay_ + dy0)            # 文字框相對錨點的縱向基準
        y0 = ay_ + dy + voff
        x0 = (ax_ + dx) if ha == "left" else (ax_ + dx - w)
        return (x0, y0, x0 + w, y0 + h)

    def _hit(a, b, p=0.0):
        return not (a[2] + p <= b[0] or b[2] + p <= a[0]
                    or a[3] + p <= b[1] or b[3] + p <= a[1])

    marker_boxes = [(px - m_half, py - m_half, px + m_half, py + m_half)
                    for px, py in pts]
    ab = ax.get_window_extent(renderer)      # 座標框：標籤不得越出

    def _inside(b):
        return (b[0] >= ab.x0 + 1.0 and b[2] <= ab.x1 - 1.0
                and b[1] >= ab.y0 + 1.0 and b[3] <= ab.y1 - 1.0)
    placed = []
    moved = 0
    for i, ann, dx0, dy0, ha0 in anns:
        bb0 = ann.get_window_extent(renderer)
        ax_, ay_ = float(pts[i][0]), float(pts[i][1])
        chosen = None
        for k, (dx, dy, ha) in enumerate(_cands(dx0, dy0, ha0)):
            bx = _box(bb0, ax_, ay_, dx0, dy0, ha0, dx, dy, ha)
            if not _inside(bx):
                continue
            if any(_hit(bx, q, pad) for q in placed):
                continue
            if any(_hit(bx, mb) for j, mb in enumerate(marker_boxes) if j != i):
                continue
            chosen = (dx, dy, ha, bx, k)
            break
        if chosen is None:
            dx, dy, ha = _cands(dx0, dy0, ha0)[-1]
            bx = _box(bb0, ax_, ay_, dx0, dy0, ha0, dx, dy, ha)
            chosen = (dx, dy, ha, bx, -1)
        dx, dy, ha, bx, k = chosen
        if k != 0:
        ...
        placed.append(bx)
    if moved:
        print(f"[PCA] 標籤避讓：{moved}/{len(anns)} 個改位")
...
def plot_pca_biplot(doc_vectors, feature_names, doc_names, outpath,
    ...
    # ---- 2. PCA 降維 ----
    # small pipe 5.1: 固定 full SVD 求解器與種子。此前未指定時 sklearn 對
    # 32×2 萬維矩陣自動選 randomized SVD 且無種子，PC 座標逐次運行漂移
    # （弟子傳 PC1 實測 0.595–0.610），圖 3 不可精確復現；full 爲精確解。
    ...
    info_text = (
        ...
        "━━━━━━━━━━━━━━━━━━\n"
        "投影方式：對中心化後的 TF-IDF 特徵矩陣作主成分分解（$\\tilde{X}=U\\Sigma V^{\\top}$，$Z=U_2\\Sigma_2$），\n"
        "取方差最大的兩個方向作爲座標軸。\n"
        "載荷向量（箭頭）：箭頭方向和長度表示該 n-gram 對主成分的貢獻大小，指向某群文本代表其高頻特徵。\n"
        "解釋方差比：PC1+PC2 合計越高，二維投影越能代表原始高維特徵空間。\n"
        "解讀：距離近=TF-IDF 特徵相似；箭頭指向=區分性 n-gram。"
    )
    ...
    # 標籤偏移（5.3：最右緣 12% 區間的點標籤改畫在點左側並右邊距加寬，
    # 防「衣鏡·弟子傳」等長標籤衝出坐標框）
    x_min_c, x_max_c = float(coords[:, 0].min()), float(coords[:, 0].max())
    x_thresh = x_max_c - 0.12 * max(x_max_c - x_min_c, 1e-9)
    _anns = []
    for i, name in enumerate(doc_names):
        short = _image_label_short(name, ascii_dot=True)
        if coords[i, 0] >= x_thresh:
            _dx0, _dy0, _ha0 = -6.0, 4.0, "right"
        else:
            _dx0, _dy0, _ha0 = 6.0, 4.0, "left"
        _a = ax.annotate(short, (coords[i, 0], coords[i, 1]),
                         textcoords="offset points", xytext=(_dx0, _dy0), ha=_ha0,
                         fontsize=text_fs, fontproperties=body_fp_pca, alpha=0.9,
                         fontweight="bold", zorder=5)
        _anns.append((i, _a, _dx0, _dy0, _ha0))
    ax.margins(x=0.06)
    ...
    # 標籤避讓須在座標範圍、載荷箭頭、圖例全部就位之後執行；若在箭頭之前
    # 執行，後續自動縮放會推移座標，算好的位置隨即失效（5.4）。
    _pca_relax_labels(fig, ax, coords, _anns, marker_size, pca_vis_scale)
    fig.savefig(outpath, dpi=_CLUSTER_EXPORT_DPI)
    plt.close(fig)
...
def plot_network_graph(sim_matrix, doc_names, outpath,
    ...
    if legend_handles:
        # 5.2b：lower left 會壓住《本姓解》節點（seed=42 佈局其恰在左下），
        # 左上象限無節點，圖例移此。
        ax.legend(handles=legend_handles, loc="upper left", fontsize=8,
                  prop=fp, framealpha=0.9, handlelength=1.5, handleheight=1.0)
...
def plot_centrality_bars(centrality_df, outpath, font_setting=""):
    ...
    info_text = (
        ...
        "━━━━━━━━━━━━━━━━━━\n"
        "公式：$\\mathrm{Strength}(i)=\\sum_{j\\ne i}\\cos_{\\mathrm{raw}}(i,j)$。即節點 i 與所有其他節點的餘弦相似度之和。\n"
        "解讀：值越高表示與其他文本的總體相似度越高，排名靠前節點可視爲語料庫中的核心文本。\n"
...
def append_clustering_section(doc, sim_matrix, doc_names, plot_paths,
    ...
    p.add_run("Ward 法通過最小化合併後的組內方差來逐步構建層次聚類結構。"
              "方差在歐氏空間中定義，故距離矩陣取 L2 歸一化向量間的歐氏距離："
              "dist(i,j) = sqrt(2(1 − cos_sim(i,j)))（弦距離）。")

    dist = 1.0 - np.array(sim_matrix, dtype=float)
    np.fill_diagonal(dist, 0)
    dist = np.clip(dist, 0, None)
    dist = np.sqrt(2.0 * dist)  # small pipe 5: 弦距離（Ward 需歐氏度量）
    condensed = squareform(dist, checks=False)
    Z = linkage(condensed, method="ward")

    sim_np = np.array(sim_matrix, dtype=float)
    for k in [2, 5]:
        labels = fcluster(Z, t=k, criterion="maxclust")
        doc.add_paragraph(f"k={k} 分群結果：", style="Normal")
        for c in sorted(set(labels)):
            idxs = [i for i in range(len(doc_names)) if labels[i] == c]
            members = [doc_names[i] for i in idxs]
            if len(idxs) > 1:
                sub = sim_np[np.ix_(idxs, idxs)]
                mmean = (float(sub.sum()) - len(idxs)) / (len(idxs) * (len(idxs) - 1))
                head = f"群 {c}（n={len(idxs)}，簇內平均相似度={mmean:.4f}）"
            else:
                head = f"群 {c}（n=1）"
            doc.add_paragraph(f"{head}：{', '.join(members)}", style="Normal")
...
def run_clustering_standalone(csv_path, out_dir, font_setting=_CLUSTER_DEFAULT_FONT):
    ...
    path_dendrogram = os.path.join(out_dir, f"{ts_prefix}_圖4.1 Ward層次聚類樹狀圖（歐氏距離，L2歸一化）.png")
...
def _diff_summary_han(
    a_text: str,
    b_text: str,
    max_items: int = 12,
    merge_repeated_variants: bool = False,
) -> Dict[str, object]:
    dist, ops = _levenshtein_diff_ops(a_text, b_text)
    rep, ins, dele = _diff_counts(ops)
    # small pipe 5.3: replace 塊按字拆爲逐字位條目（塊內逐字 1:1，A/B 段
    # 等長），delete/insert 成句塊不拆。merge_repeated_variants（默認
    # False）：默認輸出逐字位原始 notes（忠實機器輸出原則，重出項不合
    # 併）；置 True 時同一 (tag, A段, B段) 合併爲一條 note，計次 >=2 追加
    # 「（凡N見）」，順序按首次出現位置，max_items 按合併後條數截斷。
    # 僅影響展示 notes，不動任何計數。
    note_units: List[Tuple[str, str, str]] = []
    for tag, aa, bb in ops:
        if tag == "equal":
            continue
        saa = str(aa or "")
        sbb = str(bb or "")
        if tag == "replace" and len(saa) == len(sbb) and len(saa) > 1:
            note_units.extend(("replace", ca, cb) for ca, cb in zip(saa, sbb))
        else:
            note_units.append((str(tag), saa, sbb))

    note_counts: Dict[Tuple[str, str, str], int] = {}
    if merge_repeated_variants:
        note_order: List[Tuple[str, str, str]] = []
        for key in note_units:
            if key not in note_counts:
                note_counts[key] = 0
                note_order.append(key)
            note_counts[key] += 1
        display_units = note_order
    else:
        display_units = note_units

    notes: List[str] = []
    extra_notes = 0
    for tag, aa, bb in display_units:
        if tag == "replace":
