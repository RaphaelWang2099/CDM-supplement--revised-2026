# =====================================================================
# pipeline21.1 (2026-08-02) — 基於 pipeline21 的 Reranker 工程修復版。
# 論文刊面已整列刪除 Reranker（sidecar 不入任何融合分數/排名），本版
# 修復僅面向 GitHub 開源倉庫的可復現性，不影響任何刊用數值：
#   1. BGE-reranker 加載改回 AutoModelForSequenceClassification 並補導入
#      （pipeline21 僅導入 CausalLM，選 BGE 模型必 NameError）。
#   2. Qwen3-Reranker 打分：max_length 默認 512→4096；並按官方模型卡
#      process_inputs 作 suffix 保留式截斷——prefix/suffix 先行編碼，
#      截斷只作用於中段內容，<think> 答題位永不被截。
#   3. 模型缺失靜默回落 0.6B 時，行級 reranker_model_label 同步實際模型，
#      避免與 reranker_model 列不一致。
# ---------------------------------------------------------------------
# pipeline21.2 (2026-08-03) — 校勘記同字重出合併。
#   Word 報告「校勘记」展示行：同一 (tag, A段, B段) 的操作合併爲一條，
#   出現次數 ≥2 時追加「（凡N見）」；輸出順序按首次出現位置。replace
#   塊按字拆分計次（塊內逐字 1:1），delete/insert 成句塊不拆。
#   僅影響展示行；dist/rep/ins/dele 與 norm_edit 等一切計數不變
#   （仍按字符位逐位計）。
...
NORMAL_COLLATION_DEFAULTS = {
    ...
    # 默認 False：校勘記逐字位原始條目（忠實機器輸出）；True 時同項合併並記「（凡N見）」。
    "merge_repeated_variants": False,
}
...
PIPELINE_CODE_VERSION = "pipeline21_reranker_causal_v1"
...
def _load_bge_reranker_backend() -> dict:
    ...
    try:
        import torch
        # BGE-reranker-base 是序列分類架構，須用 AutoModelForSequenceClassification；
        # 2026-08-02 修復：此前僅導入 AutoModelForCausalLM，選 BGE 模型必 NameError。
...
def _load_reranker_backend(model_size: str) -> dict:
    ...
    rr_prefix = ("<|im_start|>system\nJudge whether the Document meets the requirements based on "
                 ...
                 "<|im_end|>\n<|im_start|>user\n")
    rr_suffix = "<|im_end|>\n<|im_start|>assistant\n<think>\n\n</think>\n\n"
    # 官方用法：prefix/suffix 先行編碼，截斷只作用於中段內容，保證 suffix
    # （含 <think> 與答題位）永不被截；見 Qwen3-Reranker 模型卡 process_inputs。
    rr_prefix_ids = tokenizer.encode(rr_prefix, add_special_tokens=False)
    rr_suffix_ids = tokenizer.encode(rr_suffix, add_special_tokens=False)
    backend = {
        "torch": torch,
        "tokenizer": tokenizer,
        "model": model,
        ...
        "rr_prefix": rr_prefix,
        "rr_suffix": rr_suffix,
        "rr_prefix_ids": rr_prefix_ids,
        "rr_suffix_ids": rr_suffix_ids,
    }
    _RERANK_BACKEND_CACHE[cache_key] = backend
    return backend
...
def _score_reranker_pairs(
    ...
    model_size: str,
    batch_size: int = 8,
    # 2026-08-02 修復：默認 512 會把每側 1200 字的長對整體右截，連同 suffix
    # （<think> 答題位）一起截掉，P(yes) 失去官方語義。每側 1200 字 + 模板約
    # 需 2600+ token，取 4096 以完整容納；Qwen 分支另做 suffix 保留式截斷。
    max_length: int = 4096,
    prompt: str = "",
    ...
    for i in range(0, len(pairs), bs):
        chunk = pairs[i: i + bs]
        if backend.get("token_true_id") is not None:
            instruct = str(prompt).strip() or "判断文本B与文本A在字面与语义上的整体相似程度"
            # 官方 process_inputs 口徑：prefix/suffix 已在加載時單獨編碼，
            # 截斷只作用於中段內容（Instruct/Query/Document），suffix 的
            # <think> 答題位永不被截，末位 token 恆爲答題位。
            prefix_ids = backend["rr_prefix_ids"]
            suffix_ids = backend["rr_suffix_ids"]
            content_budget = max(64, ml - len(prefix_ids) - len(suffix_ids))
            content_texts = [
                f"<Instruct>: {instruct}\n<Query>: {a}\n<Document>: {b}"
                for a, b in chunk
            ]
            with torch.no_grad():
                ...
                rows_idx = torch.arange(logits.size(0), device=logits.device)
                last_logits = logits[rows_idx, last_idx, :]
                pair2 = torch.stack([last_logits[:, backend["token_false_id"]],
                                     last_logits[:, backend["token_true_id"]]], dim=1)
                pair2 = torch.nn.functional.log_softmax(pair2.float(), dim=1)
                out_scores.extend([float(x) for x in pair2[:, 1].exp().detach().cpu().tolist()])
            continue
...
def annotate_reranker_sidecar(
    ...
    for i, idx in enumerate(sel):
        ...
        # 靜默回落（如 8B 缺失→0.6B）時，行級標籤與實際所用模型保持一致
        r["reranker_model_label"] = _reranker_model_label(str(model_used))
...
def write_normal_collation_section(
    ...
    for shown, item in enumerate(detailed_items, start=1):
        ...
        # pipeline21.2: replace 塊按字拆爲逐字位條目（replace 操作逐字
        # 1:1，塊內 A/B 段等長）；delete/insert 的成句塊不拆。
        # merge_repeated_variants（默認 False）：默認輸出逐字位原始條目
        # （忠實機器輸出原則，重出項不合併）；置 True 時同一 (tag, A段,
        # B段) 合併爲一條展示行，計次 >=2 追加「（凡N見）」，順序按首次
        # 出現位置（連書重字如「杚杚」作「紇紇」得「杚作紇（凡2見）」）。
        # 僅影響展示，不動任何計數。
        var_units: List[Tuple[str, str, str]] = []
        for tag, aa, bb in ops:
            if tag == "equal":
                continue
            saa = str(aa or "")
            ...
            if tag == "replace" and len(saa) == len(sbb) and len(saa) > 1:
                var_units.extend(("replace", ca, cb) for ca, cb in zip(saa, sbb))
            else:
                var_units.append((str(tag), saa, sbb))

        var_counts: Dict[Tuple[str, str, str], int] = {}
        if merge_repeated_variants:
            var_order: List[Tuple[str, str, str]] = []
            for key in var_units:
                if key not in var_counts:
                    var_counts[key] = 0
                    var_order.append(key)
                var_counts[key] += 1
            display_units = var_order
        else:
            display_units = var_units
        ...
        for tag, aa, bb in display_units:
            if tag == "replace":
                ...
                line = f"A有B无此{kind}：\"{aa}\""
            else:  # insert
                kind = _variant_unit_label("", bb)
                line = f"B增此{kind}：\"{bb}\""
            if merge_repeated_variants:
                cnt = int(var_counts[(tag, aa, bb)])
                if cnt >= 2:
                    line += f"（凡{cnt}見）"
...
def write_word_report(
    ...
    if fw == "merge" and gg == "normal":
        write_normal_collation_section(
            ...
            merge_repeated_variants=bool(NORMAL_COLLATION_DEFAULTS["merge_repeated_variants"]),
        )
